...
fun MainScreen(
    ...
    onDownload: (enqueue: Boolean) -> Unit,
    ...
    Box(modifier) {
        ...
        if (!showIntroTutorial) {
            MainScreenControls(
                ...
                showOverlaySelector = shownBottomSheet == null && showOverlaySelector,
                nearbyQuests = nearbyQuests,
                showQuestDetails = { mainBottomSheetViewModel.showQuest(it.key) },
    ...
    if (confirmReplaceDownload) {
        ConfirmReplaceDownloadDialog(
            ...
            onConfirmed = { onDownload(false) },
            onEnqueue = { onDownload(true) },
        )
    }
