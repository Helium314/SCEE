...
import de.westnordost.streetcomplete.ui.common.quest.elementWithoutAccessTagsFilter
import de.westnordost.streetcomplete.ui.common.quest.pathsAndRoadsFilter
...
fun OverlayForm(
    ...
    fun createDefaultOtherAnswers(): List<AnswerItem> {
        ...
        if (element != null) {
            ...
            if (prefs.expertMode /* todo && overlay != CustomOverlay -> where to get current overlay? it's in container...*/) {
                result.add(AnswerItem(stringResource(Res.string.quest_generic_answer_show_edit_tags)) { on(Action.EditTags) })
            }
            if (prefs.expertMode && pathsAndRoadsFilter.matches(element)) {
                val res = if (element.tags.containsAnyKey(*accessKeys.toTypedArray())) Res.string.manage_access
                    else Res.string.add_access
                result.add(AnswerItem(stringResource(res)) { on(Action.ManageAccess) })
            }
            if (prefs.expertMode && elementWithoutAccessTagsFilter.matches(element)
                    && pathsAndRoadsFilter.matches(element)) {
                result.add(AnswerItem(stringResource(Res.string.quest_construction)) { on(Action.UnderConstruction) })
            }
